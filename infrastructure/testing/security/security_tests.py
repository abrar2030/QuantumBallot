...
class SecurityTestSuite:
    ...
    def run_all_tests(self) -> Dict:
        ...
        print(f"Starting comprehensive security testing for {self.environment} environment")
                    ...
                    for ip_range in rule.get('IpRanges', []):
                        if ip_range.get('CidrIp') == '0.0.0.0/0':
                            if rule.get('FromPort') == 22:  # SSH
                                results["failed"].append(f"SG {sg_id} allows SSH from anywhere")
                            elif rule.get('FromPort') == 3389:  # RDP
                                results["failed"].append(f"SG {sg_id} allows RDP from anywhere")
                            elif rule.get('FromPort') not in [80, 443]:
                                results["warnings"].append(f"SG {sg_id} allows {rule.get('FromPort')} from anywhere")
            
            # Test NACLs
            nacls = ec2.describe_network_acls()['NetworkAcls']
            for nacl in nacls:
                if not nacl['IsDefault']:
                    results["passed"].append(f"Custom NACL {nacl['NetworkAclId']} configured")
    ...
    def test_iam_security(self) -> Dict:
        ...
        try:
            ...
            for policy in policies:
                ...
                for statement in policy_doc.get('Statement', []):
                    if statement.get('Effect') == 'Allow':
                        ...
                        if '*' in actions:
                            results["failed"].append(f"Policy {policy['PolicyName']} grants admin access")
                        
                        # Check for dangerous actions
                        dangerous_actions = ['iam:*', 'sts:AssumeRole', 'ec2:*']
                        for action in actions:
                            if any(dangerous in action for dangerous in dangerous_actions):
                                results["warnings"].append(f"Policy {policy['PolicyName']} has privileged action: {action}")
            ...
            for user in users:
                ...
                if not mfa_devices:
                    results["warnings"].append(f"User {username} does not have MFA enabled")
                else:
            ...
            try:
                ...
                if password_policy.get('MinimumPasswordLength', 0) < 12:
                    results["failed"].append("Password policy requires less than 12 characters")
                else:
                    results["passed"].append("Password policy meets length requirements")
                
                if not password_policy.get('RequireSymbols', False):
                    results["warnings"].append("Password policy does not require symbols")
        ...
        try:
            ...
            for bucket in buckets:
                ...
                try:
                    ...
                    results["passed"].append(f"S3 bucket {bucket_name} has encryption enabled")
                except ClientError:
                    results["failed"].append(f"S3 bucket {bucket_name} does not have encryption enabled")
        ...
        try:
            ...
            if not trails:
                ...
                for trail in trails:
                    ...
                    if status['IsLogging']:
                        results["passed"].append(f"CloudTrail {trail_name} is actively logging")
                    else:
                        results["failed"].append(f"CloudTrail {trail_name} is not logging")
                    
                    # Check if trail includes global events
                    if trail.get('IncludeGlobalServiceEvents', False):
                        results["passed"].append(f"CloudTrail {trail_name} includes global events")
                    else:
                        results["warnings"].append(f"CloudTrail {trail_name} does not include global events")
            ...
            for log_group in log_groups:
                ...
                if retention and retention >= 365:
                    results["passed"].append(f"Log group {group_name} has adequate retention")
                else:
                    results["warnings"].append(f"Log group {group_name} has insufficient retention")
        ...
        try:
            ...
            for vault in backup_vaults:
                ...
                if vault.get('EncryptionKeyArn'):
                    ...
                    results["failed"].append(f"Backup vault {vault_name} is not encrypted")
        ...
        try:
            ...
            for image in images:
                ...
                if user == 'root' or user == '0':
                    ...
                    results["passed"].append(f"Image {image_name} runs as non-root user: {user}")
                ...
                for port in exposed_ports:
                    ...
                    if port_num < 1024:
                        results["warnings"].append(f"Image {image_name} exposes privileged port {port}")
                
                # Check for secrets in environment variables
                env_vars = config.get('Env', [])
                for env_var in env_vars:
                    env_lower = env_var.lower()
                    if any(secret in env_lower for secret in ['password', 'secret', 'key', 'token']):
                        results["failed"].append(f"Image {image_name} may contain secrets in env vars")
                ...
                for image in images:
                    if image.tags:
                        ...
                        if trivy_result.returncode == 0:
                            ...
                            if critical_vulns > 0:
                                results["failed"].append(f"Image {image_name} has {critical_vulns} critical vulnerabilities")
                            elif high_vulns > 0:
                                results["warnings"].append(f"Image {image_name} has {high_vulns} high vulnerabilities")
                            else:
                                results["passed"].append(f"Image {image_name} has no high/critical vulnerabilities")
        ...
        try:
            ...
            for container in containers:
                ...
                if 'no-new-privileges:true' in security_opt:
                    results["passed"].append(f"Container {container_name} has no-new-privileges")
                else:
                    results["warnings"].append(f"Container {container_name} missing no-new-privileges")
                ...
                if user == 'root' or user == '0':
                    results["failed"].append(f"Container {container_name} running as root")
                else:
                    results["passed"].append(f"Container {container_name} running as user: {user}")
                ...
                if 'ALL' in cap_drop:
                    results["passed"].append(f"Container {container_name} drops all capabilities")
                elif cap_drop:
                    results["passed"].append(f"Container {container_name} drops capabilities: {cap_drop}")
                else:
                    results["warnings"].append(f"Container {container_name} does not drop capabilities")
                
                if cap_add:
                    results["warnings"].append(f"Container {container_name} adds capabilities: {cap_add}")
                
                # Check read-only filesystem
                read_only = container.attrs.get('HostConfig', {}).get('ReadonlyRootfs', False)
                if read_only:
                    results["passed"].append(f"Container {container_name} has read-only filesystem")
                else:
                    results["warnings"].append(f"Container {container_name} has writable filesystem")
            ...
            for namespace in namespaces.items:
                ...
                for pod in pods.items:
                    ...
                    if security_context:
                        ...
                        results["warnings"].append(f"Pod {pod_name} has no security context")
                    ...
                    for container in pod.spec.containers:
                        ...
                        if container.security_context:
                            ...
                            if sc.run_as_non_root:
                                results["passed"].append(f"Container {container_name} runs as non-root")
                            
                            if sc.read_only_root_filesystem:
                                results["passed"].append(f"Container {container_name} has read-only filesystem")
                            
                            if sc.allow_privilege_escalation is False:
                                results["passed"].append(f"Container {container_name} prevents privilege escalation")
                            else:
                                results["warnings"].append(f"Container {container_name} allows privilege escalation")
                        else:
                            results["warnings"].append(f"Container {container_name} has no security context")
                        
                        # Check resource limits
                        if container.resources:
                            if container.resources.limits:
                                results["passed"].append(f"Container {container_name} has resource limits")
                            else:
                                results["warnings"].append(f"Container {container_name} has no resource limits")
            
            # Test Network Policies
            try:
                network_policies = client.NetworkingV1Api().list_network_policy_for_all_namespaces()
                if network_policies.items:
                    results["passed"].append(f"Found {len(network_policies.items)} network policies")
                else:
                        ...
                        for port in scan_result['scan'][host]['tcp']:
                            ...
                            if state == 'open':
                                if port in [22, 3389]:  # SSH, RDP
                                    results["warnings"].append(f"Administrative port {port} ({service}) open on {host}")
                                elif port in [80, 443, 3000, 9090, 3001]:  # Web services
                                    results["passed"].append(f"Web service port {port} ({service}) open on {host}")
                                else:
                                    results["warnings"].append(f"Port {port} ({service}) open on {host}")
                ...
                with socket.create_connection((host, port), timeout=10) as sock:
                    with context.wrap_socket(sock, server_hostname=host) as ssock:
                        ...
                        if cert_obj.not_valid_after > now:
                            results["passed"].append(f"Certificate for {host}:{port} is valid")
                        else:
                            results["failed"].append(f"Certificate for {host}:{port} is expired")
                        
                        # Check certificate expiry warning
                        days_until_expiry = (cert_obj.not_valid_after - now).days
                        if days_until_expiry < 30:
                            results["warnings"].append(f"Certificate for {host}:{port} expires in {days_until_expiry} days")
                        
                        # Check TLS version
                        tls_version = ssock.version()
                        if tls_version in ['TLSv1.2', 'TLSv1.3']:
                            results["passed"].append(f"{host}:{port} uses secure TLS version: {tls_version}")
                        else:
                            results["failed"].append(f"{host}:{port} uses insecure TLS version: {tls_version}")
        ...
        for endpoint in endpoints:
            try:
                ...
                for header, expected_value in security_headers.items():
                    if header in headers:
                        if expected_value and headers[header] != expected_value:
                            results["warnings"].append(f"{endpoint} has incorrect {header}: {headers[header]}")
                        else:
                ...
                if 'Server' in headers:
                    ...
                    if any(server in server_header.lower() for server in ['nginx', 'apache', 'iis']):
                        results["warnings"].append(f"{endpoint} discloses server information: {server_header}")
                
                # Check for HTTPS redirect
                if endpoint.startswith('http://') and response.status_code not in [301, 302, 307, 308]:
                ...
                for payload in sql_payloads:
                    try:
                        ...
                        if any(pattern in response_text for pattern in error_patterns):
                            results["failed"].append(f"{endpoint} may be vulnerable to SQL injection")
                            break
                    except requests.RequestException:
                        pass
                else:
                    results["passed"].append(f"{endpoint} appears protected against SQL injection")
                ...
                for payload in xss_payloads:
                    try:
                        ...
                        if payload in response.text:
                            results["failed"].append(f"{endpoint} may be vulnerable to XSS")
                            break
                    except requests.RequestException:
                        pass
                else:
                    results["passed"].append(f"{endpoint} appears protected against XSS")
    ...
    def test_data_protection(self) -> Dict:
        ...
        results["warnings"].append("Data classification compliance needs manual verification")
        
        # Test access controls
        results["warnings"].append("Access control compliance needs manual verification")
    ...
    def test_audit_requirements(self) -> Dict:
        ...
        results["passed"].append("Audit logging tests covered in infrastructure security")
    ...
    def test_retention_policies(self) -> Dict:
        ...
        results["passed"].append("Log retention tests covered in infrastructure security")
    ...
    def test_dependency_vulnerabilities(self) -> Dict:
        ...
        try:
            ...
            if npm_audit.returncode == 0:
                ...
                if critical_vulns > 0:
                    results["failed"].append(f"Found {critical_vulns} critical npm vulnerabilities")
                elif high_vulns > 0:
                    results["warnings"].append(f"Found {high_vulns} high npm vulnerabilities")
                else:
    ...
    def test_infrastructure_vulnerabilities(self) -> Dict:
        ...
        results["warnings"].append("Infrastructure vulnerability scanning needs integration with security tools")
    ...
    def test_configuration_vulnerabilities(self) -> Dict:
        ...
        results["warnings"].append("Configuration vulnerability scanning needs implementation")
...
def main():
    ...
    parser.add_argument('--test-type', choices=['all', 'infrastructure', 'containers', 'network', 'application', 'compliance', 'vulnerabilities'], 
    ...
    if args.test_type == 'all':
        ...
        suite.generate_security_report({'tests': results, 'environment': args.environment, 'timestamp': datetime.utcnow().isoformat()})
