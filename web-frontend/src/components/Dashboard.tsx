...
function Dashboard() {
    ...
    axios.get('http://' + GLOBAL_VARIABLES.LOCALHOST + '/api/blockchain/get-results-computed')
      .then(response => {
        ...
        if (results !== undefined && results.candidatesResult) {
            let newDataCandidates = results.candidatesResult.map((x: any, index: number) => {
  ...
  }, [imageList, provinces, setData, setDataResults, setMapData, setPartiesData, setPercentage, setTopVotesPerProvinces]);
