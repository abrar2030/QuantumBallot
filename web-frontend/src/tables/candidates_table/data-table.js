...
export function DataTable({ columns, data, }) {
    ...
    return (_jsxs("div", { className: "", children: [_jsxs("div", { className: "flex gap-2 justify-between py-4", children: [_jsx(Input, { placeholder: "Filter candidate name ...", value: table.getColumn("name")?.getFilterValue() ?? "", onChange: (event) => table.getColumn("name")?.setFilterValue(event.target.value), className: "max-w-lg" }), _jsx(Input, { placeholder: "Filter code candidate ...", value: table.getColumn("code")?.getFilterValue() ?? "", onChange: (event) => table.getColumn("code")?.setFilterValue(event.target.value), className: "max-w-md" }), _jsx(Input, { placeholder: "Filter party acronym...", value: table.getColumn("acronym")?.getFilterValue() ?? "", onChange: (event) => table.getColumn("acronym")?.setFilterValue(event.target.value), className: "max-w-md" })] }), _jsx("div", { className: "rounded-md border", children: _jsxs(Table, { children: [_jsx(TableHeader, { children: table.getHeaderGroups().map((headerGroup) => (_jsx(TableRow, { children: headerGroup.headers.map((header) => {
