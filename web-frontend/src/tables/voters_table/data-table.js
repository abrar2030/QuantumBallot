...
export function DataTable({ columns, data, }) {
    ...
    return (_jsxs("div", { className: "", children: [_jsxs("div", { className: "flex gap-2 justify-between py-4", children: [_jsx(Input, { placeholder: "Filter identifiers ...", value: table.getColumn("identifier")?.getFilterValue() ?? "", onChange: (event) => table.getColumn("identifier")?.setFilterValue(event.target.value), className: "max-w-lg" }), _jsx(Input, { placeholder: "Filter Electoral number ...", value: table.getColumn("electoralId")?.getFilterValue() ?? "", onChange: (event) => table.getColumn("electoralId")?.setFilterValue(event.target.value), className: "max-w-md" }), _jsx(Input, { placeholder: "Filter Verification ...", value: table.getColumn("secret")?.getFilterValue() ?? "", onChange: (event) => table.getColumn("secret")?.setFilterValue(event.target.value), className: "max-w-md" })] }), _jsx("div", { className: "rounded-md border", children: _jsxs(Table, { children: [_jsx(TableHeader, { children: table.getHeaderGroups().map((headerGroup) => (_jsx(TableRow, { children: headerGroup.headers.map((header) => {
