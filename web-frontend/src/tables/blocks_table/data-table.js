...
export function DataTable({ columns, data, }) {
    ...
    return (_jsxs("div", { className: "", children: [_jsxs("div", { className: "flex gap-2 justify-between py-4", children: [_jsx(Input, { placeholder: "Filter Hash Block ...", value: table.getColumn("hashBlock")?.getFilterValue() ?? "", onChange: (event) => table.getColumn("hashBlock")?.setFilterValue(event.target.value), className: "max-w-lg" }), _jsx(Input, { placeholder: "Filter Number of Transactions ...", value: table.getColumn("numOfTransactions")?.getFilterValue() ?? "", onChange: (event) => table.getColumn("numOfTransactions")?.setFilterValue(event.target.value), className: "max-w-md" })] }), _jsx("div", { className: "rounded-md border", children: _jsxs(Table, { children: [_jsx(TableHeader, { children: table.getHeaderGroups().map((headerGroup) => (_jsx(TableRow, { children: headerGroup.headers.map((header) => {
